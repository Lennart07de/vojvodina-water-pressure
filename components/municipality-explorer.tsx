...
export default function MunicipalityExplorer({districtId,districtName,selected,onSelect,onData}:{districtId:string;districtName:string;selected:string;onSelect:(id:string)=>void;onData:(data:MunicipalResponse)=>void}){
  ...
  return <section className='municipality-explorer detail-panel' id='municipal-outlook' aria-label='Municipal monitoring details'>
    <span className='eyebrow'>Municipality · {districtName}</span>
    <p className='small-muted'>Approximately 9 km weather grid. Nearby municipalities may share forecasts; check actual local supply before acting.</p>
    ...
    <label className='municipality-select'>Select municipality / city in {districtName}<select value={selected} onChange={e=>onSelect(e.target.value)}>{rows.map(a=><option key={a.id} value={a.id}>{a.name} — {usable?.municipalities.find(m=>m.id===a.id)?.grade??'No data'}</option>)}</select></label>
      <article className='municipality-detail' aria-live='polite'><span className='eyebrow'>MUNICIPALITY / CITY DETAIL</span><h3>{area.name}</h3><span className={`badge ${(result?.grade??'missing').toLowerCase()}`}>{result?.grade?`${result.grade} weather stress`:loading?'Loading':'No data'}</span>
        <dl><div><dt>Seven-day mean air temperature</dt><dd>{fmt(result?.temperature)} °C</dd></div><div><dt>Weather pressure score</dt><dd>{fmt(result?.score,0)} / 100</dd></div><div><dt>Seven-day precipitation</dt><dd>{fmt(result?.precipitation)} mm</dd></div><div><dt>Seven-day reference ET₀</dt><dd>{fmt(result?.et0)} mm</dd></div><div><dt>Area-weighted deficit</dt><dd>{fmt(result?.deficit)} mm</dd></div></dl>
