...
function edgeCache():EdgeCache|undefined {return (globalThis.caches as unknown as {default?:EdgeCache}|undefined)?.default;}
async function readEdo(now:Date):Promise<Edo>{
  ...
  try{
    const r=await fetch('https://drought.emergency.copernicus.eu/api/wms?SERVICE=WMS&REQUEST=GetCapabilities&VERSION=1.1.1',{signal:AbortSignal.timeout(12000)});
    if(!r.ok)return base;
...
async function readRiver(now:Date):Promise<River>{
  ...
  try{const r=await fetch(RIVER_SOURCE,{signal:AbortSignal.timeout(12000)});return r.ok?parseRiver(await r.text(),now):empty;}catch{return empty;}
}
...
export async function GET(request:Request){
  const now=new Date(),cacheKey=new Request(new URL('/api/pressure?schema=1&window='+forecastWindow(now).start,request.url)),cache=edgeCache();
  if(memory&&memory.expires>now.getTime()&&isUsable(memory.result,now))return Response.json(memory.result);
  if(cache){try{const cached=await cache.match(cacheKey);if(cached){const value=await cached.json() as PressureResponse;if(isUsable(value,now)){memory={result:value,expires:Math.min(Date.parse(value.retrievedAt)+21600000,now.getTime()+600000)};return Response.json(value);}}}catch{/* Continue with live data. */}}
  ...
  try{
    ...
    if(cache){try{await cache.put(cacheKey,Response.json(result,{headers:{'Cache-Control':`public, max-age=${ttl}`}}));}catch{/* Nonessential cache. */}}
    return Response.json(result,{headers:{'Cache-Control':'no-store'}});
  }catch{
    if(memory&&isUsable(memory.result,now))return Response.json({...memory.result,warning:'Refresh failed. Showing the previous, unexpired forecast.'});
    return Response.json({error:'Forecast service unavailable. No pressure scores can be calculated.'},{status:503});
  }
}
